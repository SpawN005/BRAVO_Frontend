...
import observerService from '@/services/observer/observerService'; // Update the path accordingly
...
const ObserversTable = () => {
  ...
  useEffect(() => {
    const fetchObservers = async () => {
      try {
        const response = await observerService.getAllObservers();
  ...
  const handleDelete = async (observerId:any) => {
    ...
    .then(async (willDelete) => {
      if (willDelete) {
        ...
        swal("Your observer data is safe!");
      }
    });
  };
...
router.push(`/observers/update/${observerId}`);
  };
